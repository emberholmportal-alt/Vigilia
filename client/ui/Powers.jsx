...
import { SKILLS, SKILL_LABEL, SKILL_DESC, SKILL_CAP, skillXpForLevel } from '../data/progression.js'
  ...
  return (
    <div className="modal-backdrop" onClick={() => setPanel(null)}>
      <div className="flare-panel" style={{ backgroundImage: `url(${UI}powers.png)` }}
        ...
        <div className="char-title" style={{ left: '50%', top: (24 / PH * 100) + '%', transform: 'translate(-50%,-50%)', position: 'absolute' }}>Acciones</div>
            ...
            return (
              <div className="pw-row" key={k}>
                <div className="pw-head">
                  <b>{SKILL_LABEL[k]}</b>
                  <span>Nv {sk.level}<em>/{SKILL_CAP}</em></span>
                </div>
                <div className="pw-desc">{SKILL_DESC[k]}</div>
